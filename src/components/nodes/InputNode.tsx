...
export const InputNode: React.FC<InputNodeProps> = ({ id, data, selected }) => {
        ...
        <div
            className={clsx(
                'relative group min-w-[100px] rounded-lg border-2 transition-all duration-300 backdrop-blur-md shadow-lg',
                {
                    ...
                    'bg-slate-800 border-emerald-500/50 shadow-emerald-500/10 scale-105': selected && !isBiasProvider,
                    'border-emerald-500/50 shadow-emerald-500/10': !selected && !isBiasProvider,
                    'bg-slate-800 border-yellow-500/50 shadow-yellow-500/10 scale-105': selected && isBiasProvider,
                ...
                <div className="mt-0.5 bg-slate-900/50 rounded flex flex-row justify-between items-center px-2 py-1.5 border border-slate-700/50 gap-3">
                    <div className="flex flex-row items-center justify-between w-full gap-2">
                        ...
                        <span className={clsx(
                            ...
                            isBiasProvider ? "text-yellow-400" : "text-emerald-400"
                        )}>
