...
import type { McCullochPitts } from '../../models/neural';
import type { NeuronNodeData } from '../NetworkCanvas/types';

interface McCullochPittsNodeProps {
    id: string;
    data: NeuronNodeData;
    selected: boolean;
}

export const McCullochPittsNode: React.FC<McCullochPittsNodeProps> = ({ id, data, selected }) => {
    const neuron = data.neuron as McCullochPitts;
        ...
        <div
            className={clsx(
                'relative group min-w-[100px] rounded-lg border-2 transition-all duration-300 backdrop-blur-md shadow-lg',
                {
                    ...
                    'bg-slate-800 border-indigo-500 shadow-indigo-500/20 scale-105': selected,
                    'border-indigo-500/50 shadow-indigo-500/10': !selected,
                }
            )}
        >
            <button
            ...
            <Handle
                ...
                id="input"
                className="!w-4 !h-4 !bg-slate-800 !border-2 !border-indigo-400 hover:!bg-indigo-400 !transition-colors"
            />

            <div className="p-1.5 flex flex-col gap-1">
                <div className="flex items-center gap-1.5">
                    <div className="flex items-center justify-center w-5 h-5 rounded-full ring-2 ring-offset-1 ring-offset-slate-900 bg-indigo-500/20 text-indigo-400 ring-indigo-500/50">
                    ...
                    <div className="flex flex-col flex-1">
                        ...
                        <span className={clsx(
                            "text-[12px] font-mono font-bold font-black leading-none",
                            {
                                ...
                                "text-indigo-400": (neuron.output as number) > 0,
                            }
                        )}>
            ...
            <Handle
                ...
                className="!w-4 !h-4 !bg-slate-800 !border-2 !transition-colors !border-indigo-400 hover:!bg-indigo-400"
            />
        </div>
    );
};
